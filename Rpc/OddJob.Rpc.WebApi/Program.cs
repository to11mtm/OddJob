...
namespace OddJob.Rpc.WebApi
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            {
                var cs =
                    SqlConnectionHelper.CheckConnString("rpcsample", "f:\\",
                        false);
                SqlServerUnitTestTableHelper.EnsureTablesExist("rpcsample",
                    "f:\\");
                using (var conn =
                ...
                container.Register<IJobQueueDbConnectionFactory>(() =>
                    new TestDbConnectionFactory("f:\\", "rpcsample"));
            ...
            else
            {
                ...
                var cmd =
                    SQLiteUnitTestTableHelper.heldConnection.CreateCommand();
                var tableHelper = new SqlTableHelper(
                    new SQLiteJobQueueDataConnectionFactory(
                        SQLiteUnitTestTableHelper
                            .connString), new SQLiteGenerator());
                tableHelper
                    .GetCreateMainTableIndexes(
                        new SqlDbJobQueueDefaultTableConfiguration()).ForEach(
                        t =>
                        {
                            cmd.CommandText = t;
                            cmd.ExecuteNonQuery();
            ...
            container
                .Register<ISerializedJobQueueAdder, BaseSqlJobQueueAdder>();
            container
                .Register<ISqlDbJobQueueTableConfiguration,
                    SqlDbJobQueueDefaultTableConfiguration>();
            container
                .Register<IJobAdderQueueTableResolver,
                    DefaultJobAdderQueueTableResolver>();
            container
                .Register<IJobTypeResolver, NullOnMissingTypeJobTypeResolver>();
